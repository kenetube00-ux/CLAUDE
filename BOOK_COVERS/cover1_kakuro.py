...
def create_cover():
    ...
    pdf.add_text(175, 715, "LARGE PRINT EDITION", font='F2', size=20)

    # Main title
    pdf.add_centered_text(640, "KAKURO", font='F2', size=56)
    pdf.add_centered_text(585, "PUZZLE BOOK", font='F2', size=38)
    pdf.add_centered_text(535, "For Seniors", font='F1', size=22)
    ...
    for row in range(5):
        for col in range(5):
            ...
            if (row + col) % 3 == 0:
                ...
                pdf.add_text(x + 5, y + cell - 14, "12", font='F1', size=9, gray=1)
                pdf.add_line(x, y + cell, x + cell, y, 0.5, gray=0.7)
            else:
                pdf.add_rect(x, y, cell, cell, line_width=0.8)

    # Features
    pdf.add_centered_text(230, "100+ Puzzles | Bold & Easy | Big Grids", font='F1', size=13)
    pdf.add_centered_text(205, "Clear Numbers | Senior-Friendly Format", font='F1', size=13)

    # Volume
    pdf.add_centered_text(160, "Volume 1", font='F1', size=14)

    # Author
    pdf.add_line(180, 110, 432, 110, 1)
    pdf.add_centered_text(80, "Daniel Tesfamariam", font='F2', size=20)
