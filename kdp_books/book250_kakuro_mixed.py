"""Book 250: Kakuro Puzzle Book - Mixed Difficulty Volume 1"""
import random
import os, sys
...
def generate_kakuro_puzzle(grid_size):
    ...
    for i in range(grid_size):
        grid[0][i] = 'clue'
        grid[i][0] = 'clue'
    clue_prob = 0.3 if grid_size <= 6 else 0.27
    ...
    for row in range(grid_size):
        for col in range(grid_size):
            ...
            if grid[row][col] == 'clue':
                ...
                max_clue = 24 if grid_size <= 6 else (35 if grid_size <= 7 else 45)
                if col < grid_size - 1 and grid[row][col+1] == 'empty':
                    across = random.randint(3, max_clue)
                    pdf.add_text(cx + cell_size*0.52, cy - cell_size*0.85, str(across), font='F2', size=7, gray=1)
                if row < grid_size - 1 and grid[row+1][col] == 'empty':
                    down = random.randint(3, max_clue)
...
pdf.add_centered_text(550, "KAKURO PUZZLE BOOK", font='F2', size=28)
pdf.add_centered_text(500, "Mixed Difficulty Volume 1", font='F2', size=20)
pdf.add_centered_text(420, "25 Puzzles: Easy + Medium + Hard", font='F1', size=16)
pdf.add_centered_text(380, "8 Easy (5x5) + 9 Medium (7x7) + 8 Hard (9x9)", font='F1', size=13)
pdf.add_centered_text(100, "Daniel Tesfamariam", font='F2', size=14)

pdf.new_page()
pdf.add_centered_text(600, "KAKURO PUZZLE BOOK - Mixed Difficulty Volume 1", font='F2', size=13)
...
pdf.add_text(72, 510, "This book progresses from easy to hard:", font='F2', size=13)
pdf.add_text(72, 480, "Puzzles 1-8: EASY (5x5 grids, small sums)", font='F1', size=12)
pdf.add_text(72, 455, "Puzzles 9-17: MEDIUM (7x7 grids, moderate sums)", font='F1', size=12)
pdf.add_text(72, 430, "Puzzles 18-25: HARD (9x9 grids, large sums)", font='F1', size=12)
...
for puzzle_num in range(25):
    ...
    pdf.add_centered_text(715, f"Difficulty: {difficulties[puzzle_num]} ({gs}x{gs})", font='F1', size=12)
    start_x = (612 - gs * cell_size) / 2
    start_y = 660
    draw_kakuro_grid(pdf, gs, start_x, start_y, cell_size)
...
for sol_page in range(5):
    ...
    for p in range(min(5, 25 - start_idx)):
        ...
        gs = grid_sizes[idx]
        small_cell = 20
        pdf.add_text(sx, sy + 14, f"#{idx+1} ({difficulties[idx]})", font='F2', size=9)
        for r in range(gs):
            for c in range(gs):
                cx = sx + c * small_cell
                cy = sy - r * small_cell
...
pdf.save(os.path.join(os.path.dirname(os.path.abspath(__file__)), "Book250_Kakuro_Mixed.pdf"))
print("Book250_Kakuro_Mixed.pdf created successfully!")
